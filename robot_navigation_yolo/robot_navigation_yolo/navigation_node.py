...
from nav2_msgs.action import NavigateToPose  # Para usar o Nav2
...
class NavigationRobot(Node):
    ...
    def __init__(self):
        ...
        # Ajuste para a assinatura de imagem
        ...
        self.current_image = None  # Variável para armazenar a última imagem recebida

    def detector_callback(self, msg):
        """ Interrompe a exploração e define um objetivo quando uma pessoa for encontrada """
        if not self.result_detector:
            self.result_detector = True
            self.get_logger().info(f"Pessoa encontrada em: {msg.data}")
            
            # Interromper exploração
            stop_msg = Bool()
            stop_msg.data = True
            self.stop_exploration_publisher.publish(stop_msg)
            
            try:
                # Assumindo que a mensagem msg.data contém x, y, z diretamente
                pose = json.loads(msg.data)
                self.get_logger().info(f"msg.data: {msg.data}")
                if len(pose) == 1 and len(pose[0]) == 3:
                    x, y, z = pose[0]
                
                # Posicionamento no mapa (ajuste de escala)
                map_x = x   # Ajuste de escala para o mapa
                map_y = y  # Ajuste de escala para o mapa
                
                goal_msg = NavigateToPose.Goal()
                
                # Aqui, verifique se a imagem foi recebida antes de chamar a função de orientação
                if self.current_image is not None:
                    ...
                    # Definir posição do robô ao lado da pessoa com base na orientação
                    if person_status == "frente ou costas":
                        # Se a pessoa estiver de frente, posicionar à esquerda ou direita (dependendo da orientação do ângulo)
                        goal_msg.pose.pose.position.x = map_x + 0.5 * math.cos(theta)  # 0.5 metros ao lado da pessoa
                        goal_msg.pose.pose.position.y = map_y + 0.5 * math.sin(theta)  # 0.5 metros ao lado da pessoa
                    elif person_status == "de lado":
                        # Caso a pessoa esteja de lado, posicionar na frente ou nas costas (ajustar conforme o seu caso)
                        goal_msg.pose.pose.position.x = map_x + 0.5
                        goal_msg.pose.pose.position.y = map_y

                    goal_msg.pose.pose.position.z = z  # Utilizando o valor de z fornecido diretamente
                    
                    # Ajuste da orientação para o robô
    ...
    def orientation_callback(self, msg):
        """ Processa a imagem para determinar a orientação da pessoa """
        ...
        if result.pose_landmarks:
            ...
            if larg_omb > larg_quad * 1.2:
                return "frente ou costas", 0.5, theta  # Pessoa de frente ou costas
            elif larg_omb < larg_quad * 0.8:
                return "de lado", 0.0, theta  # Pessoa de lado
        
        return "Falha na deteccao", 0.0, theta

    def image_callback(self, msg):
        """ Atualiza a última imagem recebida """
